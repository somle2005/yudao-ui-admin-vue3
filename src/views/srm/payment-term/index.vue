<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="外币采购条款(英文)" prop="paymentTermEnForeign">
        <el-input
          v-model="queryParams.paymentTermEnForeign"
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['srm:payment-term:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['srm:payment-term:export']"
        >
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['srm:payment-term:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['srm:payment-term:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <PaymentTermForm ref="formRef" @success="getList" />
</template>
...
/** 付款条款 列表 */
defineOptions({ name: 'SrmPaymentTerm' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<PaymentTermVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await PaymentTermApi.getPaymentTermPage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await PaymentTermApi.exportPaymentTerm(queryParams)
    download.excel(data, '付款条款.xls')
  } catch {
  } finally {
