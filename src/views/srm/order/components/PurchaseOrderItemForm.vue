  ...
  <div style="display: contents">
    ...
    >
      ...
      <el-table border :data="formData" class="-mt-10px" @selection-change="handleSelectionChange">
          ...
          <template #default="{ row, $index }">
            ...
            >
              ...
              <SmSelect
                ...
                v-model="row.productId"
                placeholder="请选择产品编码"
                @change="(val) => changeProduct(val, row)"
                :data="productList"
        ...
        <el-table-column v-if="showOringinCount" label="数量" width="120" align="center">
          <template #default="{ row, $index }">
            <el-form-item :prop="`${$index}.qty`" class="mb-0px!">
              <!-- <el-input-number
                :disabled="countDisabled"
...
import { getDeclaredType, currencyNameChange, currencyChange } from '@/utils/operate/srm'
...
import { useSupplierProductList } from '../../common/hooks/add'
...
//   if (!['create', 'update'].includes(props.formType)) return
...
const { canSupplierProductList, canOneSupplierProductList } = useSupplierProductList(
  ['create', 'update'],
  openFormData
)

const changeProduct = (val, row) => {
  updateModelValue(
    val,
    row,
    productList,
  ...
  canOneSupplierProductList(props, row)
}

watch(
  () => openFormData.value.supplierId,
  async (val) => {
    canSupplierProductList(props, formData, val)
  },
