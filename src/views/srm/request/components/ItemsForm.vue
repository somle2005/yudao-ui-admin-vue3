    ...
    >
      ...
      <template #taxRate="{ row, $index }">
        <el-form-item :prop="`${$index}.taxRate`" class="mb-0px!">
          <SmNumber :precision="2" v-model="row.taxRate" @change="(val) => batchChange(row, val, 'taxRate')" />
        </el-form-item>
      </template>
...
// 合并采购时-产品无法变更只有供应商可变更
const { canSupplierProductList } = useSupplierProductList(
  ['merge'],
  openFormData
)
...
watch(
  ...
  async (val) => {
    console.log(val, 'val-supplierId')
