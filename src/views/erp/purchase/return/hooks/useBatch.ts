...
export const useBatch = (selectionList, getList, wholeOrderEnable, openForm) => {
  ...
  const handleSubmitAuditBatch = async () => {
    try {
      ...
      await PurchaseReturnApi.submitPurchaseReturnAudit({ ids })
  ...
  const changeRefundStatusBatch = async (list: any[], refund: boolean) => {
    ...
    try {
      ...
      // 退货单获取主Id整单退货
      await PurchaseReturnApi.changePurchaseReturnPayStatus({ 
        ids: Array.from(new Set(list.map((item) => item.id))),
        refund
      })
