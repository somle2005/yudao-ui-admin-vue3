<template>
  <el-form ref="formRef" :model="formData" :rules="formRules" label-width="180px">
    ...
    <el-row>
      <el-col :span="24">
        <el-form-item label="电子集成模块" prop="electronicIntegrationModules">
          <el-select v-model="formData.electronicIntegrationModules" clearable placeholder="请选择">
            <el-option
...
interface FormData {
  ...
  electronicIntegrationModules?: boolean | undefined;

}

defineOptions({ name: 'ProductMonitorRiser' })
...
const formData = ref<FormData>({
  ...
  electronicIntegrationModules: undefined,

})
const formRules = reactive({
  productId: [{ required: true, message: '产品主表id不能为空', trigger: 'blur' }],
  electronicIntegrationModules: [{ required: true, message: '请选择是否为电子集成模块', trigger: 'blur' }],
})
