<template>
  <el-form ref="formRef" :model="formData" :rules="formRules" label-width="180px">
    ...
    <el-row>
      <el-col :span="24">
        <el-form-item label="移动功能" prop="mobileFunction">
          <el-row>
            <el-col :span="6">
              <el-checkbox v-model="mobileFunctionShow">是否具备</el-checkbox>
            </el-col>
            <el-col :span="18" v-if="mobileFunctionShow" style="padding-left: 20px">
              <el-form-item label-width="0" prop="mobileFunction">
                <el-input
                  v-model="formData.mobileFunction"
                  type="textarea"
                  :rows="1"
                  placeholder="请输入功能描述"
                />
...
interface FormData {
  ...
  mobileFunction?: string | undefined; // 修改为仅允许字符串
  otherFeatures?: string | undefined;
}

defineOptions({ name: 'ProductTVStandWithMount' })
...
const mobileFunctionShow = ref(false)
...
onMounted(() => {
  ...
  if (formData.value.mobileFunction !== undefined && formData.value.mobileFunction.trim() !== '') {
    mobileFunctionShow.value = true
  }
...
const validateForm = async () => {
  ...
  if (mobileFunctionShow.value === false) {
    formData.value.mobileFunction = ''
  }
  // 校验表单
