...
export const FinanceSubjectApi = {
  ...
  getFinanceSubjectPage: async (params: any) => {
    return await request.get({ url: `/fms/company/page`, params })
  },

  // 查询结算账户精简列表
  getFinanceSubjectSimpleList: async () => {
    return await request.get({ url: `/fms/company/simple-list` })
  },

  // 查询Erp财务主体详情
  getFinanceSubject: async (id: number) => {
    return await request.get({ url: `/fms/company/get?id=` + id })
  },

  // 新增Erp财务主体
  createFinanceSubject: async (data: FinanceSubjectVO) => {
    return await request.post({ url: `/fms/company/create`, data })
  },

  // 修改Erp财务主体
  updateFinanceSubject: async (data: FinanceSubjectVO) => {
    return await request.put({ url: `/fms/company/update`, data })
  },

  // 删除Erp财务主体
  deleteFinanceSubject: async (id: number) => {
    return await request.delete({ url: `/fms/company/delete?id=` + id })
  },

  // 导出Erp财务主体 Excel
  exportFinanceSubject: async (params) => {
    return await request.download({ url: `/fms/company/export-excel`, params })
  }
}
