...
export const CustomRuleCategoryApi = {
  ...
  getCustomRuleCategoryPage: async (params: any) => {
    return await request.get({ url: `/tms/custom-category/page`, params })
  },

  // 查询海关品类详情
  getCustomRuleCategory: async (id: number) => {
    return await request.get({ url: `/tms/custom-category/get?id=` + id })
  },

  // 新增海关品类
  createCustomRuleCategory: async (data: CustomRuleCategoryVO) => {
    return await request.post({ url: `/tms/custom-category/create`, data })
  },

  // 修改海关品类
  updateCustomRuleCategory: async (data: CustomRuleCategoryVO) => {
    return await request.put({ url: `/tms/custom-category/update`, data })
  },

  // 删除海关品类
  deleteCustomRuleCategory: async (id: number) => {
    return await request.delete({ url: `/tms/custom-category/delete?id=` + id })
  },

  // 导出海关品类 Excel
  exportCustomRuleCategory: async (params) => {
    return await request.download({ url: `/tms/custom-category/export-excel`, params })
  },
...
// ==================== 子表（海关分类子表） ====================
  ...
  getCustomRuleCategoryItemListByCategoryId: async (categoryId) => {
    return await request.get({ url: `/tms/custom-category/custom-category-item/list-by-category-id?categoryId=` + categoryId })
  },

  // 获得海关分类组合值精简列表
  getCustomRuleCategorySimpleList: async () => {
    return await request.get({ url: `/tms/custom-category/simple-list` })
  },
}
